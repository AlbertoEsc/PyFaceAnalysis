#! /usr/bin/env python
# face_normalization_tools: functions useful to normalize facial images. This is useful to create normalized datasets
# containing centered faces, eyes, or also no-face images
#
...
def compute_approximate_mouth_coordinates(eye_coordinates):
    ...
    This function is useful when eye annotations are available, but mouth annotations are not. It assumes that 
    the face is mostly frontal (except for the screen plane rotation), and has a similar shape as computer 
    generated ones (FaceGen software).
    ...
    Returns:
        ndarray: coordinates [mouth_x, mouth_y] 
    """
...
def im_transform_randombackground(im, out_size, transf, data, filter):
    ...
    if x0 >= 0 and y0 >= 0 and x1 <= im_width and y1 <= im_height:
    ...
    # The variables {XY}p{01} contain the true destination location of the pixels to be mapped
    Xp0, Yp0, Xp1, Yp1 = 0, 0, patch_width, patch_height
...
def normalize_image(filename, coordinates, normalization_method="eyes_mouth_area", centering_mode="mid_eyes_mouth",
                    rotation_mode="noRotation", integer_rotation_center=True, out_size=(256, 192), convert_format="L",
                    verbose=False, allow_random_background=True, image=None):
    """ Opens an image "filename", normalizes its size depending on the "normalization_method", and centers it at the 
    point specified by "centering_mode". 
    Args:
        ...
        centering_mode: how the center of the image is computed
        rotation_mode: specifies how the image is rotated
        integer_rotation_center: the internal rotation uses a rotation center with integer coordinates
        out_size: the size of the output image
        convert_format: the format of the output image
        verbose: verbose
        allow_random_background: if the output image contains pixels out of the original image, use random data
        image: if this input is provided, filename is ignored, and this image is used instead
    Returns:
    ...
    inferred_mouth_x, inferred_mouth_y = compute_approximate_mouth_coordinates(
        (LeftEyeCenter_x, LeftEyeCenter_y, RightEyeCenter_x, RightEyeCenter_y))
    print "inferred_mouth_x, inferred_mouth_y=", inferred_mouth_x, inferred_mouth_y

    height_triangle_with_inferred_mouth = numpy.sqrt(
        (eyes_x_m - inferred_mouth_x) ** 2 + (eyes_y_m - inferred_mouth_y) ** 2)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        print "output_pattern -- pattern for the filename used to store the normalized images, e.g. 'image%05d.jpg'"
        print "or 'None' to preserve original filename"
        print "mode -- either 'mid_eyes_mouth' or 'mid_eyes_inferred-mouth' or 'background' or"
        print "'mid_eyes_inferred-mouthZ_horiz' or 'mid_eyes_inferred-mouthZ2_horiz' or"
        print "'mid_eyes_inferred-mouthZ2_horiz-Test'"
    ...
    if mode == "mid_eyes_mouth_horiz":  # Default for face detection (true mouth coordinates)
        ...
        prefix = "EyeN"
        num_tries = 1
        # This might introduce an additional transformation that decreases performance, instead have black background.
        allow_random_background = True and False
        ...
        if filename == "":
            ...
            if dist_eyes < 5:  # 15: #20
                ...
                for repetition in range(num_tries):
                    im2 = normalize_image(filename, float_coords, normalization_method=normalization_method,
                                          centering_mode=centering_mode, rotation_mode=rotation_mode,
                                          integer_rotation_center=integer_rotation_center, out_size=out_size,
                                          convert_format=convert_format, verbose=False,
